...
/**
 * [NEW UPGRADE]
 * SUMMARY: v2.0 Live Database CBT Engine.
 * 1. Semantic Question Grouping: Dynamically parses `config.questions` from the Neon DB, groups them strictly by `sub_topic`, and dynamically populates the subjects palette.
 * 2. Real Option Mapping: Transmutes the strict JSON options dictionary ({"A": "...", "B": "..."}) into the frontend array mapping required to render the selector UI cleanly.
 * 3. Exact Key Tracking: Tracks `userAnswers` mapped to the exact UUID `q.id` generated by Postgres, ensuring the analytics submission endpoint processes it flawlessly.
 * 4. Active AI Explanations: Pipes the genuine 550B `explanation` string directly into the Active Learning Modal when the student clicks an incorrect distractor!
 * ================================================================================================
 * 🧠 JEMER ACADEMY DESIGN SYSTEM — BRAIN TRAINING CBT SESSION (v2.0)
 * ================================================================================================
 */
...
export default function BrainTrainingSession({ config, onExit }) {
  ...
  const questionsRepo = useMemo(() => {
    ...
    questions.forEach(q => {
      ...
      // The DB provides options as {"A": "val", "B": "val"}. We map it for the UI.
  ...
  // We use the raw DB id for tracking telemetry back to backend
  ...
  return (
    <div className="w-full max-w-7xl mx-auto space-y-6 animate-fade-in pb-8 lg:pb-12 px-4 sm:px-0">
      ...
      <AiExplanations
        ...
        explanationText={`**AI Diagnostic Insight:** \n\n${currentQuestion?.explanation || "Explanation not generated."}`}
      />
