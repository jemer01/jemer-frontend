  ...
  const gaugeChartData = [
    {
      ...
      title: { text: isHunterMode ? "Hunter Mastery Index" : isStudyMode ? "Study Mastery Index" : isPracticeMode ? "Practice Readiness Index" : isWaecMode ? "WASSCE Readiness Index" : "UTME Readiness Index", font: { size: 11, color: "#64748b" } },
      gauge: {
        ...
        steps: [
          ...
          { range: [75, 100], color: isHunterMode ? "rgba(20, 184, 166, 0.12)" : isStudyMode ? "rgba(147, 51, 234, 0.12)" : isPracticeMode ? "rgba(249, 115, 22, 0.12)" : isWaecMode ? "rgba(37, 99, 235, 0.12)" : "rgba(16, 185, 129, 0.12)" },
        ],
      },
    },
  ];
  ...
  return (
    <div className="w-full max-w-6xl mx-auto space-y-6 sm:space-y-8 animate-fade-in pb-12 lg:pb-16 overflow-x-hidden">
      ...
      <div className={`relative rounded-3xl p-5 sm:p-8 bg-gradient-to-br text-white overflow-hidden shadow-2xl border ${
        isHunterMode ? "from-teal-900 via-slate-900 to-cyan-950 border-teal-500/20" :
        ...
        <div className={`absolute -top-32 -right-32 w-96 h-96 rounded-full blur-3xl pointer-events-none ${isHunterMode ? "bg-teal-500/20" : isStudyMode ? "bg-purple-500/20" : isPracticeMode ? "bg-orange-500/20" : isWaecMode ? "bg-blue-500/20" : "bg-emerald-500/20"}`} />
        <div className={`absolute -bottom-20 -left-20 w-72 h-72 rounded-full blur-3xl pointer-events-none ${isHunterMode ? "bg-cyan-500/10" : isStudyMode ? "bg-fuchsia-500/10" : isPracticeMode ? "bg-amber-500/10" : isWaecMode ? "bg-indigo-500/10" : "bg-teal-500/10"}`} />
        ...
        <div className="relative z-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-6 w-full">
          <div className="space-y-3 min-w-0 w-full md:w-auto">
            <div className={`inline-flex items-center gap-2 px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wider font-mono border ${
              isHunterMode ? "bg-teal-500/20 text-teal-300 border-teal-500/30" :
              ...
              <span className={`w-2 h-2 rounded-full shrink-0 ${isHunterMode ? "bg-teal-400" : isStudyMode ? "bg-purple-400" : isPracticeMode ? "bg-orange-400" : isWaecMode ? "bg-blue-400" : "bg-emerald-400"}`} />
              {isHistoryView 
                ? "Historical Performance Record" 
                : isHunterMode ? "Custom Hunt Completed" : isStudyMode ? "Study Session Completed" : isPracticeMode ? "Practice Session Completed" : isWaecMode ? "WASSCE Session Completed" : "JAMB CBT Session Completed"
              }
            </div>
            <h1 className="text-2xl sm:text-4xl font-display font-black tracking-tight text-white truncate">
              Candidate: <span className={isHunterMode ? "text-teal-400" : isStudyMode ? "text-purple-400" : isPracticeMode ? "text-orange-400" : isWaecMode ? "text-blue-400" : "text-emerald-400"}>John Jonathan</span>
            </h1>
            <div className="flex flex-wrap items-center gap-3 sm:gap-4 text-xs font-medium text-slate-300">
              <span className="flex items-center gap-1.5 shrink-0">
                <svg className={`w-4 h-4 shrink-0 ${isHunterMode ? "text-teal-500" : isStudyMode ? "text-purple-500" : isPracticeMode ? "text-orange-500" : isWaecMode ? "text-blue-500" : "text-emerald-500"}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          ...
          <button onClick={onRestart} className="w-full sm:w-auto px-6 py-3 rounded-xl bg-white/10 hover:bg-white/20 border border-white/20 text-white font-bold text-xs shadow-md backdrop-blur-sm transition-all active:scale-95 shrink-0 flex items-center justify-center gap-2">
            {isHistoryView ? (
              <>
        ...
        <div className="lg:col-span-5 flex flex-col gap-6">
          ...
          <div className="p-6 sm:p-8 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col items-center justify-center text-center relative overflow-hidden">
            <div className={`absolute inset-x-0 top-0 h-1 bg-gradient-to-r ${
              isHunterMode ? "from-teal-500 to-cyan-400" :
            ...
            <h2 className="text-xs font-black uppercase tracking-widest text-slate-500 dark:text-slate-400 mb-2">
              {isHunterMode ? "Hunter Session Score" : isStudyMode ? "Study Session Score" : isPracticeMode ? "Practice Score" : isWaecMode ? "WASSCE Global Aggregate" : "Aggregate UTME Score"}
            </h2>
            ...
            <div className={`mt-4 px-4 py-1.5 rounded-full text-xs font-bold border ${
              isHunterMode ? "bg-teal-50 dark:bg-teal-900/30 text-teal-700 dark:text-teal-400 border-teal-200 dark:border-teal-800/50" :
                      ...
                      {isSingleSubjectMode || isWaecMode ? (
                        <>
                          ...
                          <td className={`p-3 sm:p-4 text-xs sm:text-sm font-black font-mono text-right ${
                            ["A1", "B2", "B3"].includes(sub.grade) ? (isHunterMode ? "text-teal-600 dark:text-teal-400" : isStudyMode ? "text-purple-600 dark:text-purple-400" : isPracticeMode ? "text-orange-600 dark:text-orange-400" : "text-blue-600 dark:text-blue-400") :
                            ["C4", "C5", "C6"].includes(sub.grade) ? (isHunterMode ? "text-cyan-600 dark:text-cyan-400" : isStudyMode ? "text-fuchsia-600 dark:text-fuchsia-400" : isPracticeMode ? "text-amber-600 dark:text-amber-400" : "text-emerald-600 dark:text-emerald-400") :
        ...
        <div className="lg:col-span-7 flex flex-col gap-6">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            <div className="p-4 sm:p-5 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col h-[280px] sm:h-[300px] w-full min-w-0">
              <h3 className="text-xs sm:text-sm font-bold text-slate-900 dark:text-white mb-2 flex items-center gap-2">
                <span className={`w-2 h-2 rounded-full shrink-0 ${isHunterMode ? "bg-teal-500" : isStudyMode ? "bg-purple-500" : isPracticeMode ? "bg-orange-500" : isWaecMode ? "bg-blue-500" : "bg-emerald-500"}`} /> 
                Subject Mastery
              </h3>
            ...
            <div className="p-4 sm:p-5 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col h-[280px] sm:h-[300px] w-full min-w-0">
              <h3 className="text-xs sm:text-sm font-bold text-slate-900 dark:text-white mb-2 flex items-center gap-2">
                <span className={`w-2 h-2 rounded-full shrink-0 ${isHunterMode ? "bg-cyan-500" : isStudyMode ? "bg-fuchsia-500" : isPracticeMode ? "bg-orange-500" : "bg-indigo-500"}`} /> 
                Overall Accuracy
              </h3>
              <div className="flex-1 w-full h-full min-h-0 relative">
          ...
          <div className="p-4 sm:p-5 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col items-center justify-center h-[200px] sm:h-[220px] w-full min-w-0 relative">
            <h3 className="text-xs sm:text-sm font-bold text-slate-900 dark:text-white absolute top-4 left-5 flex items-center gap-2">
              <span className={`w-2 h-2 rounded-full shrink-0 ${isHunterMode ? "bg-teal-400" : isStudyMode ? "bg-fuchsia-400" : isPracticeMode ? "bg-amber-400" : isWaecMode ? "bg-blue-400" : "bg-teal-500"}`} /> 
              {isHunterMode ? "Hunter Mastery Index" : isStudyMode ? "Study Mastery Index" : isPracticeMode ? "Speed & Readiness Index" : "Readiness Index"}
            </h3>
      ...
      <div className={`p-6 sm:p-8 rounded-3xl bg-gradient-to-r shadow-sm relative overflow-hidden flex flex-col md:flex-row items-start md:items-center gap-6 border ${
        isHunterMode ? "from-teal-50 via-cyan-50 to-teal-50 dark:from-teal-950/40 dark:via-cyan-900/20 dark:to-teal-950/40 border-teal-200 dark:border-teal-800/50" :
        ...
        <div className={`w-14 h-14 sm:w-16 sm:h-16 rounded-2xl bg-gradient-to-br text-white flex items-center justify-center shadow-lg shrink-0 relative z-10 ${
          isHunterMode ? "from-teal-500 to-cyan-600 shadow-teal-500/30" :
